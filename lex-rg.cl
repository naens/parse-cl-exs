...
;;	     (format t "~&SUB[?]=~S" sub)
	  ...
	  ((and (equal (first e) 'Q) (equal (third e) #\*))
	   (let* ((n2 (comb pre (1+ c)))
		  ...
		  (sub (re2rgsub (second e) n (comb curr "-") 1 n (nconc rules a))))
;;	     (format t "~&SUB[*]=~S" sub)
	     (re2rg (rest re) n2 pre (2+ c) r sub)))
	  ((and (equal (first e) 'Q) (equal (third e) #\+))
	   (let* ((n2 (comb pre (1+ c)))
		  (rule (make--rules n n2))		    
		  (sub (re2rgsub (second e) (list n curr) (comb curr "-") 1 n (nconc rule a))))
;;	     (format t "~&SUB[+]=~S" sub)
	     (re2rg (rest re) n2 pre (2+ c) r sub)))
	  ((and (equal (first e) 'Q) (equal (third e) 1))
	   (let ((sub (re2rgsub (second e) curr (comb n "-") 1 n a)))
;;	     (format t "~&SUB[1]=~S" sub)
	  ...
	  ((and (equal (first e) 'Q) (numberp (third e)))
	   (labels ((do-repeat (last from to acc)
		      (if (< from to)
			  acc
			  (let ((nn (if (= from to) curr (comb curr "_" from "\\-0"))))
			    (do-repeat
			      nn
			      (1- from)
			      to
			      (re2rgsub (second e) nn (comb curr "_" from "-") 1 last acc))))))
...
(prs (str2rg "a(b(cb)*|c(bc)*)a" 'S))
(prs (str2rg "ab(cb)*a" 'S))
(prs (str2rg "ab(cb)+a" 'S))
(prs (str2rg "ab(cb)a" 'S))
(prs (str2rg "ab(cb)?a" 'S))
(prs (str2rg "ab(cb){2}a" 'S))
(prs (str2rg "a(a|b){3}b" 'S))
